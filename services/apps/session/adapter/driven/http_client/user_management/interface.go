...
type DrivenUserMgnt interface {
	...
	GetAccountInfoByAccount(ctx context.Context, account string) (resp *AccountInfoResp, err error)
}
...
type AccountInfo struct {
	...
	AuthType      string `json:"auth_type"`      // 用户认证方式 local:本地认证 domain:域认证 third:第三方认证
	DisableStatus bool   `json:"disable_status"` // 账户禁用状态 false:未禁用 true:已禁用
}

// AccountInfoResp 匹配账户信息响应
type AccountInfoResp struct {
	Result bool         `json:"result"` // 账户匹配结果。result为true时，代表匹配到相应账户；result为false时，代表未匹配到
	User   *AccountInfo `json:"user"`   // 用户信息。当result为true时，user字段有返回。
}
