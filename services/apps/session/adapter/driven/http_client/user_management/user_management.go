...
type usermgntSvc struct {
	...
	httpClientEx http_client.HTTPClient
	//mqClient     msqclient.ProtonMSQClient
	roleTypeMap map[string]RoleType
}

// NewUserMgnt 创建UserMgnt服务处理对象
func NewUserMgnt(httpClient http_client.HTTPClient) DrivenUserMgnt {
	usermgntOnce.Do(func() {
		...
		usermgnt = &usermgntSvc{
			baseURL:      fmt.Sprintf("%s/api/user-management", settings.ConfigInstance.Config.DepServices.UserMgmPrivate),
...
func (u *usermgntSvc) GetDepAllUserInfos(ctx context.Context, depID string) (userInfos []UserInfo, err error) {
	...
	respParam, err := u.httpClientEx.Get(ctx, target, nil)
	if err != nil {
		//u.log.WithContext(ctx).Errorf("GetDepAllUserInfos failed:%v, url:%v", err, target)
...
func (u *usermgntSvc) convertUserInfo(info map[string]interface{}) (userInfo UserInfo, err error) {
	...
	for _, parentDep := range parentDeps {
		...
		for _, vv := range parentDep.([]interface{}) {
			if department.PathID == "" {
				department.PathID = vv.(map[string]interface{})["id"].(string)
			} else {
				department.PathID = department.PathID + "/" + vv.(map[string]interface{})["id"].(string)
			}
			if department.Path == "" {
				department.Path = vv.(map[string]interface{})["name"].(string)
			} else {
				department.Path = department.Path + "/" + vv.(map[string]interface{})["name"].(string)
			}
		}
		userInfo.ParentDeps = append(userInfo.ParentDeps, department)
	}
...
// GetAccountInfoByAccount 根据账号匹配账户信息
func (u *usermgntSvc) GetAccountInfoByAccount(ctx context.Context, account string) (resp *AccountInfoResp, err error) {
	val := url.Values{}
	val.Add("account", account)
	val.Add("id_card_login", "false")
	val.Add("prefix_match", "false")
	target := ContactStr(u.baseURL, "/v1/account-match?", val.Encode())
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, target, http.NoBody)
	if err != nil {
		log.WithContext(ctx).Error("GetAccountInfoByAccount new req failed", zap.Error(err), zap.String("url", target))
		return nil, err
	}
	res, err := u.httpClient.GetRawClient().Do(req)
	if err != nil {
		log.WithContext(ctx).Error("GetAccountInfoByAccount do req failed", zap.Error(err), zap.String("url", target))
		return nil, err
	}
	...
	if err != nil {
		log.WithContext(ctx).Error("GetAccountInfoByAccount read resp body failed", zap.Error(err), zap.String("url", target))
		return nil, err
	}
	...
	resp = new(AccountInfoResp)
	err = json.Unmarshal(resBody, resp)
	if err != nil {
		log.WithContext(ctx).Error("GetAccountInfoByAccount unmarshal resp body failed", zap.Error(err), zap.String("url", target))
		return nil, err
	}
