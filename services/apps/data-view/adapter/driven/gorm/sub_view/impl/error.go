...
// newErrSubViewAlreadyExists 返回错误码 DataView.SubView.AlreadyExists
func newErrSubViewAlreadyExists(name string) error {
	return errorcode.SubViewAlreadyExists.Desc(name)
}

// newErrSubViewNotFound 返回错误码 DataView.SubView.NotFound
func newErrSubViewNotFound(id uuid.UUID) error {
	return errorcode.SubViewNotFound.Desc(id)
}

// newErrSubViewDatabaseError 返回错误码 DataView.SubView.DatabaseError
func newErrSubViewDatabaseError(err error) error {
	return errorcode.SubViewDatabaseError.Detail(err.Error())
}
