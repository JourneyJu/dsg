...
func ReadSheetList(excelType string, file multipart.File) ([]string, *excelize.File, *xls.WorkBook, error) {
	...
	if excelType == "xlsx" {
		...
		return sheetList, xlsxFile, nil, nil
		...
		return sheetList, nil, xlsFile, nil
	} else {
		return nil, nil, nil, errors.New("不支持的文件类型")
	}

}

func GetRows(excelType string, sheetName string, xlsxFile *excelize.File, xlsFile *xls.WorkBook) ([][]string, error) {
	if excelType == "xlsx" {
