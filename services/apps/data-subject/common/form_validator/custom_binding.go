...
var (
	customQuery         = queryBinding{}
	customForm          = formBinding{}
	customFormMultipart = formMultipartBinding{}
)
...
func (formBinding) Bind(c *gin.Context, obj any) error {
	...
	//添加路劲参数
	for _, v := range c.Params {
		c.Request.Form.Set(v.Key, v.Value)
	}
...
func (formMultipartBinding) Bind(c *gin.Context, obj any) error {
	if err := c.Request.ParseMultipartForm(defaultMemory); err != nil {
		return err
	}
	//添加路劲参数
	for _, v := range c.Params {
		c.Request.MultipartForm.Value[v.Key] = []string{v.Value}
	}

	if err := mappingByPtr(obj, (*multipartRequest)(c.Request), "form"); err != nil {
		return err
	}
...
func tryToSetValue(value reflect.Value, field reflect.StructField, setter setter, tag string) (bool, error) {
	...
	isSet, err := setter.TrySet(value, field, tagValue, setOpt)
	if err != nil {
		var validErrors ValidErrors
		validErrors = append(validErrors, &ValidError{
			Key:     registerTagName(field),
			Message: "请输入符合要求的数据类型和数据范围",
		})

		log.Errorf("err: %v, validErrors: %v", err, validErrors)
		return isSet, validErrors
	}
