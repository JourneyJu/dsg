...
INSERT INTO `configuration` (`key`,`value`,`type`)
SELECT '产业大脑', 'https://cydn-demo.kweaver-ai.cn/home/homePage', '1'
FROM DUAL WHERE NOT EXISTS(SELECT `key` FROM `configuration` WHERE `key` = '产业大脑' );

INSERT INTO `configuration` (`key`,`value`,`type`)
SELECT '指标溯源与影响分析', 'https://anyfabric-demo.kweaver-ai.cn/anyfabric/data-catalog/developing', '1'
FROM DUAL WHERE NOT EXISTS( SELECT `key` FROM `configuration` WHERE `key` = '指标溯源与影响分析' );

INSERT INTO `configuration` (`key`,`value`,`type`)
SELECT '流程可视化监控与分析', 'https://anyfabric-demo.kweaver-ai.cn/anyfabric/data-catalog/developing', '1'
...
CREATE TABLE IF NOT EXISTS  `app` (
    ...
    `apps_id` CHAR(36) NOT NULL DEFAULT UUID()  COMMENT '对象ID, uuid',
