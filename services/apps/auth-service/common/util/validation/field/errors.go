...
func NotSupported[T ~string](field *Path, value interface{}, validValues []T) *Error {
	...
	if len(validValues) > 0 {
		...
		detail = fmt.Sprintf("%s 必须是 %s 中的一个", field, strings.Join(stringValues, ", "))
	}
...
func TooLong(field *Path, value interface{}, maxLength int) *Error {
	return &Error{ErrorTypeTooLong, field.String(), value, fmt.Sprintf("%s 长度必须不超过 %d", field, maxLength)}
}
...
func (list ErrorList) Is(target error) bool {
	_, ok := target.(*ErrorList)
	return ok
}
