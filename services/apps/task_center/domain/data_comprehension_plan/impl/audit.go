...
func (d *DataComprehensionPlan) DataComprehensioPlanAuditResultMsgProc(ctx context.Context, msg *wf_common.AuditResultMsg) (err error) {
	// 获取最终结果的消息处理
	var (
		plans              []*model.DataComprehensionPlan
	...
	if plans, err = d.comprehensionPlanRepo.GetByUniqueIDs(ctx, []uint64{planID}); err != nil {
		log.WithContext(ctx).
	...
	err = d.comprehensionPlanRepo.Update(ctx, plans[0])
	if err != nil {
		return err
	}
...
func (d *DataComprehensionPlan) DataComprehensionPlanAuditProcessMsgProc(ctx context.Context, msg *wf_common.AuditProcessMsg) (err error) {
	// 获取审核原因的消息处理 todo
	var (
		plans              []*model.DataComprehensionPlan
		planID, auditRecID uint64
...
func (d *DataComprehensionPlan) Cancel(ctx context.Context, id string) (err error) {
	...
	if err = d.wf.AuditCancel(
		&wf_common.AuditCancelMsg{
			ApplyIDs: []string{GenAuditApplyID(modelPlan.DataComprehensionPlanID, uint64(*modelPlan.AuditID))},
			Cause: struct {
...
func (d *DataComprehensionPlan) AuditList(ctx context.Context, query *domain_plan.AuditListGetReq) (*domain_plan.ComprehensionPlanAuditListResp, error) {
	var (
		err    error
		audits *workflow.AuditResponse
	)

	audits, err = d.wfRest.GetList(ctx, workflow.WorkflowListType(query.Target), []string{workflow.AF_TASKS_DATA_COMPREHENSION_PLAN}, query.Offset, query.Limit, query.Keyword)
	if err != nil {
		log.WithContext(ctx).Errorf("uc.workflow.GetList failed: %v", err)
		return nil, errorcode.Detail(errorcode.InternalError, err)
	}
	resp := &domain_plan.ComprehensionPlanAuditListResp{}
	resp.TotalCount = int64(audits.TotalCount)
	resp.Entries = make([]*domain_plan.ComprehensionAuditPlanItem, 0, len(audits.Entries))
	for i := range audits.Entries {
		...
		resp.Entries = append(resp.Entries,
			&domain_plan.ComprehensionAuditPlanItem{
