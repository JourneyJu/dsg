...
func (d *DataAggregationPlan) DataAggregationPlanAuditResultMsgProc(ctx context.Context, msg *wf_common.AuditResultMsg) (err error) {
	// 获取最终结果的消息处理
	var (
		plans              []*model.DataAggregationPlan
	...
	if plans, err = d.aggregationPlanRepo.GetByUniqueIDs(ctx, []uint64{planID}); err != nil {
		log.WithContext(ctx).
	...
	err = d.aggregationPlanRepo.Update(ctx, plans[0])
	if err != nil {
		return err
	}
...
func (d *DataAggregationPlan) DataAggregationPlanAuditProcessMsgProc(ctx context.Context, msg *wf_common.AuditProcessMsg) (err error) {
	// 获取审核原因的消息处理 todo
	var (
		plans              []*model.DataAggregationPlan
		planID, auditRecID uint64
	...
	if !(plans[0].AuditStatus != nil && *plans[0].AuditStatus == domain_plan.Auditing) {
		log.WithContext(ctx).
			Warnf("plan: %d status is not AUDITING, ignore it", plans[0].ID)
		return nil
	}
...
func (d *DataAggregationPlan) Cancel(ctx context.Context, id string) (err error) {
	...
	if err = d.wf.AuditCancel(
		&wf_common.AuditCancelMsg{
			ApplyIDs: []string{GenAuditApplyID(modelPlan.DataAggregationPlanID, uint64(*modelPlan.AuditID))},
			Cause: struct {
...
func (d *DataAggregationPlan) AuditList(ctx context.Context, query *domain_plan.AuditListGetReq) (*domain_plan.AggregationPlanAuditListResp, error) {
	var (
		err    error
		audits *workflow.AuditResponse
	)

	audits, err = d.wfRest.GetList(ctx, workflow.WorkflowListType(query.Target), []string{workflow.AF_TASKS_DATA_AGGREGATOPM_PLAN}, query.Offset, query.Limit, query.Keyword)
	if err != nil {
		log.WithContext(ctx).Errorf("uc.workflow.GetList failed: %v", err)
		return nil, errorcode.Detail(errorcode.InternalError, err)
	}
	resp := &domain_plan.AggregationPlanAuditListResp{}
	resp.TotalCount = int64(audits.TotalCount)
	resp.Entries = make([]*domain_plan.AggregationAuditPlanItem, 0, len(audits.Entries))
	for i := range audits.Entries {
		...
		resp.Entries = append(resp.Entries,
			&domain_plan.AggregationAuditPlanItem{
