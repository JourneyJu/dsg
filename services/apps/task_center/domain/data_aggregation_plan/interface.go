...
type DataAggregationPlan interface {
	Delete(ctx context.Context, Id string) error
	Create(ctx context.Context, req *AggregationPlanCreateReq, userId, userName string) (*IDResp, error)
	Update(ctx context.Context, req *AggregationPlanUpdateReq, id, userId, userName string) (*IDResp, error)
	GetById(ctx context.Context, id string) (*AggregationPlanGetByIdReq, error)
	List(ctx context.Context, query *AggregationPlanQueryParam) (*AggregationPlanListResp, error)
	CheckNameRepeat(ctx context.Context, req *AggregationPlanNameRepeatReq) error
	Cancel(ctx context.Context, Id string) error
	AuditList(ctx context.Context, query *AuditListGetReq) (*AggregationPlanAuditListResp, error)
...
func (f *AggregationPlanCreateReq) ToModel(userId string, uniqueID uint64) *model.DataAggregationPlan {
	var autoStart int8
	if f.AutoStart {
		autoStart = 1
	}
	plan := &model.DataAggregationPlan{
		DataAggregationPlanID: uniqueID,
		ID:                    uuid.NewString(),
		Name:                  f.Name,
...
// 更新
type AggregationPlanUpdateReq struct {
	Name            string `json:"name" form:"name" binding:"required,trimSpace,min=1,max=128"`               // 名称
	ResponsibleUID  string `json:"responsible_uid" binding:"required_if=NeedDeclaration true,omitempty,uuid"` // 责任人
	Priority        string `json:"priority" binding:"required_if=NeedDeclaration true,omitempty"`             // 优先级【1: 普通， 2：紧急， 3：非常紧急】
	StartedAt       int64  `json:"started_at" binding:"max=9999999999" example:"4102329600"`                  // 开始日期
...
// 详情
type AggregationPlanGetByIdReq struct {
...
// 列表
type AggregationPlanQueryParam struct {
	...
	UserID       string `json:"user_id" form:"user_id" binding:"omitempty"`                                             // 用户ID过滤条件
}
...
type AggregationPlanListResp struct {
	PageResult[AggregationPlanItem]
}

type AggregationPlanItem struct {
...
// 名称重复检查
type AggregationPlanNameRepeatReq struct {
...
type AggregationPlanAuditListResp struct {
	PageResult[AggregationAuditPlanItem]
}

type AggregationAuditPlanItem struct {
	ID            string `json:"id"`              // 计划id
	Name          string `json:"name"`            // 名称
