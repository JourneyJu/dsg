...
const TableNameDataAggregationPlan = "data_aggregation_plan"

// DataAggregationPlan mapped from table <data_aggregation_plan>
type DataAggregationPlan struct {
	DataAggregationPlanID uint64                `gorm:"column:data_aggregation_plan_id" json:"data_aggregation_plan_id"`           // 雪花id
	...
	AutoStart             *int8                 `gorm:"column:auto_start;not null" json:"auto_start"`                              // 是否自动启动，枚举：0：不自动启动，1：自动启动
	...
	Status                *int                  `gorm:"column:status;not null" json:"status"`                                      // 状态【1:未开始  2: 进行中  3: 已完成】
...
func (m *DataAggregationPlan) BeforeCreate(_ *gorm.DB) error {
	if m == nil {
		return nil
	}

	if m.DataAggregationPlanID == 0 {
		m.DataAggregationPlanID, _ = utilities.GetUniqueID()
	}
...
// TableName DataAggregationPlan's table name
func (*DataAggregationPlan) TableName() string {
	return TableNameDataAggregationPlan
}
