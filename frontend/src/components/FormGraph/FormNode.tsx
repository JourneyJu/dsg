...
const FormNodeComponent = (props: any) => {
    ...
    const versionParams = useMemo(() => {
        ...
        const dragData = getDragData ? getDragData(data?.fid) : {}
    ...
    }, [isDraft, selectedVersion, getDragData, data?.fid])
    ...
    const handleCopy = () => {
        ...
        const selectedFieldsData = selectedFields.filter((selectedField) => {
            return !unQuoteFields.find(
                (unQuoteField) => unQuoteField.id === selectedField.id,
            )
        })
        setSelectedFields(selectedFieldsData)
        node.replaceData({
