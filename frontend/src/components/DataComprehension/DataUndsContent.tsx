...
const DataUndsContent = () => {
    ...
    const originType = query.get('originType') || ''
    ...
    const back = () => {
        ...
        if (backUrl) {
            ...
            // 数据资源目录的理解
            if (originType) {
                url = `${backUrl}?tabKey=${originType}`
            } else {
                url = `/complete-work-order-task?projectId=${projectId}&taskId=${taskId}&backUrl=${backUrl}`
            }
            // window.location.replace(url)
            navigate(url)
        } else {
            navigate('/comprehensionManage/dataComprehensionReport')
        }
    }
    ...
    const handleReturn = () => {
        ...
        if (viewMode === ViewMode.EDIT) {
            ReturnConfirmModal({
                content: __(
                    '离开此页将放弃本次更改的内容，若当前正在 AI 理解中，再次返回您需重新发起，请确认操作！',
                ),
                onCancel: back,
            })
        } else {
                ...
                <Col className={styles.topRight} span={8}>
                    {/* <Button
                        ...
                        hidden={
                            ...
                            !checkPermission('viewDataUnderstandingReport')
                        }
                    >
                        {__('编辑')}
                    </Button> */}
