...
import { OpAssessmentPlanTypeOptions, PlanListUseSceneEnum } from '../const'
...
const PlanList = forwardRef(
    (
        ...
        const dataProcessingRef = useRef<any>()
        const dataUnderstandingRef = useRef<any>()
        ...
        useImperativeHandle(ref, () => ({
            getFormData: async () => {
                const dataAcquisitionData =
                    (await dataAcquisitionRef.current?.getFormData()) || []
                const dataUnderstandingData =
                    (await dataUnderstandingRef.current?.getFormData()) || []
                const dataProcessingData =
                    (await dataProcessingRef.current?.getFormData()) || []
                if (
                    dataAcquisitionData === 'error' ||
                    dataUnderstandingData === 'error' ||
                    dataProcessingData === 'error'
                ) {
                    return 'error'
                }
                return {
                    operation_data_collection: dataAcquisitionData?.map(
                        (item) => ({
                            id: item.id,
                    ...
                    operation_data_understanding: dataUnderstandingData?.map(
                        (item) => ({
                            id: item.id,
                            data_understanding_actual: Number(
                                item.data_understanding_actual || 0,
                            ),
                        }),
                    ),
                    operation_data_process: dataProcessingData?.map((item) => ({
                        id: item.id,
                        data_process_explore_actual: Number(
                            item.data_process_explore_actual || 0,
                        ),
                        data_process_fusion_actual: Number(
                            item.data_process_fusion_actual || 0,
                        ),
                    })),
                }
            },
        ...
        const getEndedHeader = (
            planItem: (typeof OpAssessmentPlanTypeOptions)[number],
            ...
            <div className={styles['plan-list']}>
                ...
                {!dataSource || dataSource.length === 0 ? (
                    ...
                    <Collapse
                        bordered={false}
                        collapsible="icon"
                        defaultActiveKey={OpAssessmentPlanTypeOptions.map(
                            (item) => item.id,
                        )}
                        className={styles['custom-collapse']}
                    >
                        {OpAssessmentPlanTypeOptions.map((item) => {
