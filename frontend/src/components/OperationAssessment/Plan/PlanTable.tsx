...
type TableItem = ITargetEvaluationPlanItem & {
    ...
    data_process_explore_actual?: number | string
    data_process_fusion_actual?: number | string
    data_understanding_actual?: number | string
}
...
const PlanTable = forwardRef(
    (
        ...
        useImperativeHandle(ref, () => ({
            getFormData: async () => {
                try {
                    ...
                    // 返回完整的数据，而不是筛选后的数据
                    return updatedTableData.map((item, index) => ({
        ...
        useEffect(() => {
            const data: TableItem[] = dataSource.map(
                (item) =>
                    ({
                        ...item,
                        actual_quantity:
                            planType === AssessmentPlanTypeEnum.DataAcquisition
                                ? item.actual_collection_count || ''
                                : '',
                        data_process_explore_actual:
                            planType === AssessmentPlanTypeEnum.DataProcessing
                                ? item.data_process_explore_actual || ''
                                : '',
                        data_process_fusion_actual:
                            planType === AssessmentPlanTypeEnum.DataProcessing
                                ? item.data_process_fusion_actual || ''
                                : '',
                        data_understanding_actual:
                            planType ===
                            AssessmentPlanTypeEnum.DataUnderstanding
                                ? item.data_understanding_actual || ''
                                : '',
                    } as TableItem),
            )
            setTableData(data)
            ...
            const formData = data.map((item) => {
                const formItem: any = { ...item }
                if (planType === AssessmentPlanTypeEnum.DataAcquisition) {
                    formItem.actual_quantity = item.actual_quantity
                } else if (planType === AssessmentPlanTypeEnum.DataProcessing) {
                    formItem.data_process_explore_actual =
                        item.data_process_explore_actual
                    formItem.data_process_fusion_actual =
                        item.data_process_fusion_actual
                } else if (
                    planType === AssessmentPlanTypeEnum.DataUnderstanding
                ) {
                    formItem.data_understanding_actual =
                        item.data_understanding_actual
                }
                return formItem
            })
        ...
        const columns = () => {
            ...
            if (useScene === PlanListUseSceneEnum.TargetEvaluation) {
                ...
                const dataProcessingEvaluationColumns = [
                    ExploreNumColumn,
                    {
                        title: (
                            <TitleFilter
                ...
                const dataUnderstandingEvaluationColumns = [
                    ...DataUnderstandingColumns,
                    {
                        title: (
                            <TitleFilter
                ...
                const EvaluationColumnsMap = {
                    ...
                    [AssessmentPlanTypeEnum.DataProcessing]:
                        dataProcessingEvaluationColumns,
                    [AssessmentPlanTypeEnum.DataUnderstanding]:
                        dataUnderstandingEvaluationColumns,
                }
