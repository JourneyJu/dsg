...
import {
    ...
    ViewMode,
    viewModeItems,
} from '@/components/BusiArchitecture/const'
import CreateArchitecture from '@/components/BusiArchitecture/CreateArchitecture'
import { getPlatformNumber, OperateType } from '@/utils'
import Confirm from '@/components/Confirm'
import Details from '@/components/BusiArchitecture/Details'
import Move from '@/components/BusiArchitecture/Move'
...
import InfoSystem from '@/components/DataSource/InfoSystem'
import {
    AuditStatusTag,
    getDisabledTooltip,
...
function CombedCard({
    ...
    useEffect(() => {
        // 查询所有时更新
        if (!searchCondition.parent_id && !isSearch) {
            const isAllPass = data?.every(
                (o) => o?.audit_status === BusinessAuditStatus.Published,
            )
            setTaskInfo((prev) => ({
                ...prev,
                isAllPass,
    ...
    }, [data, searchCondition, isSearch])

    // 获取业务流程树
    const getProcessTree = async () => {
        try {
            setFetching(true)
            const { parent_id, keyword, info_related, model_related } =
                searchCondition
    ...
    const getProcessList = async () => {
        try {
            ...
            const res = await getBusinessDomainProcessList(params)
            setData(res.entries)
            setTotal(res.total_count)
            setFetching(false)
        } catch (error) {
    ...
    const getArchitechListByInfoSys = async () => {
        try {
            ...
            const res = await getInfoSysProcessList({
                ...rest,
                task_id: taskInfo?.id,
            })
            setData(res.entries)
    ...
    const getOptionMenus = (record: IBusinessDomainItem) => {
        ...
        return optionMenus
            .filter((op) => {
                ...
                if (!canOpt) {
                    return op.key === OperateType.DETAIL
                }
    ...
    return (
        <div className={styles.combedCardWrapper}>
...
export default memo(CombedCard)
