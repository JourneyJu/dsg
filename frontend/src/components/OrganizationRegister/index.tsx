...
import OrganizationDetail from './operate/Detail'
...
const OrganizationRegister: React.FC = () => {
    ...
    // 当前操作项
    const [operateItem, setOperateItem] = useState<IObject>()
    ...
    const getTableList = async (params: any) => {
        try {
            ...
            const res = await getObjectsOrganization(params)
    ...
    const getTopOperate = () => {
        return (
            <>
                <div className={styles.title}>{__('机构注册')}</div>
    ...
    return (
        <div className={styles.organizationRegisterWrapper}>
            {loading ? (
                renderLoader()
            ) : (
                <>
